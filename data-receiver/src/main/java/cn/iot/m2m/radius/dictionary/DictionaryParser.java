...
import cn.iot.m2m.radius.attribute.*;
...
public class DictionaryParser {
    ...
    public static void parseDictionary(InputStream source, WritableDictionary dictionary)
        ...
        while ((line = in.readLine()) != null) {
            ...
            if ("ATTRIBUTE".equalsIgnoreCase(lineType))
                parseAttributeLine(dictionary, tok, lineNum);
            else if ("VALUE".equalsIgnoreCase(lineType))
                parseValueLine(dictionary, tok, lineNum);
            else if ("$INCLUDE".equalsIgnoreCase(lineType))
                includeDictionaryFile(dictionary, tok, lineNum);
            else if ("VENDORATTR".equalsIgnoreCase(lineType))
                parseVendorAttributeLine(dictionary, tok, lineNum);
            else if ("VENDOR".equalsIgnoreCase(lineType))
    ...
    private static Class<?> getAttributeTypeClass(int attributeType, String typeStr) {
        ...
        else if ("octets".equalsIgnoreCase(typeStr))
            type = RadiusAttribute.class;
        else if ("integer".equalsIgnoreCase(typeStr)||"date".equalsIgnoreCase(typeStr))
            type = IntegerAttribute.class;
        else if ("ipaddr".equalsIgnoreCase(typeStr))
